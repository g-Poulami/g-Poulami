...
- **[Genomic-Sequence-Variation-Analysis](https://github.com/g-Poulami/Genomic-Sequence-Variation-Analysis)**: How does genetic variation differ across individuals and populations — and what can those differences tell us about ancestry, population structure, and evolutionary history? An end-to-end R pipeline for analysing SNP variation from multi-sample VCF input, designed for two distinct genomic domains: bacterial genomics and ancient human DNA (aDNA). Covers quality filtering (QUAL, depth, missingness, MAF), PCA with LD pruning, pairwise Weir & Cockerham FST, ADMIXTURE parsing and CV-error selection of optimal K, neighbour-joining phylogenetics with bootstrap support, and aDNA-specific processing (pseudo-haploid calling, C>T/G>A deamination reporting, mapDamage2 integration). Produces publication-quality figures and structured output files across seven modular R scripts orchestrated by a single master pipeline.
